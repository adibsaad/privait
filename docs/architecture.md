...
| Projects scope chats and knowledge, not a KNN rewrite     | `conversations.project_id` + `files.project_id` add a scoping column instead of new vector plumbing: project grounding reuses the global KNN and filters to the project's knowledge files app-side (same exact-at-desktop-scale pattern as per-chat scoping). Global memories still reach project chats — one memory store by design.                                                                                                                                                                                            |
| Memories are inspectable rows, not a vector sidecar       | A regular `memories` table (id, content, source, provenance, timestamps) with a separate vec0 index keeps every memory visible, editable, and deletable — nothing stored about the user is hidden. Post-chat distillation writes through the same inspectable layer. Incognito chats read no memories, write none, and stay out of transcript search. The memory threshold is a per-turn settings value; transcript search runs full-text in place over `messages_fts` (raw transcripts are never copied into the memory layer). |
| Distillation reconciles, not appends (task 0029, planned) | An append-only memory store accumulates contradictions ("lives in NYC" + "lives in SF"), and retrieval has no recency sense to arbitrate them. The distiller is offered the recent store and may rewrite or remove superseded `distilled` memories — through the same inspectable rows, with each change enumerated in the chat's tool step. What the user wrote by hand is only changed by the user.                                                                                                                            |
