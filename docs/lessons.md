...
- FTS5 external-content tables: queries WITHOUT a MATCH clause (e.g. `id NOT IN (SELECT rowid FROM messages_fts)`) scan the CONTENT table, so they "find" unindexed rows — only a `MATCH` query proves what's actually indexed. After adding such a table, the backfill (`INSERT INTO t(t) VALUES ('rebuild')`) must ship in the SAME migration or old rows stay invisible; append it as a new migration if the creator already shipped.
